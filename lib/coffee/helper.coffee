...
exports.format_date = (date, friendly)->
	...
	if friendly
		...
		if mseconds < time_std[4]
			...
			return Math.floor(mseconds / time_std[2]).toString() + ' 小时前' if mseconds > time_std[2] && Math.floor(mseconds / time_std[2]) < 24
			return Math.floor(mseconds / time_std[3]).toString() + ' 天前' if mseconds > time_std[2]

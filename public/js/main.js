...
--------------------------------------------
 */
var DMHandler, Giccoo, SHAKE_THRESHOLD, checkForm, checkTimes, closeAll, createForm, deviceMotionHandler, editadr, gico, gotopage, hideAll, hidecancel, isScrolling_move, last_update, last_x, last_y, last_z, limitposts, loginsubmit, myFunction, readygo, ressubmit, showactivetable, showcancel, showsecondbar, signpop, signup, _x, _y, _z;
...
checkTimes = function() {
  return $("[data-time]").each(function() {
    var day, hour, miao, mint, now, test;
    ...
    if (test > new Date()) {
      ...
      if (day === 0 && hour === 0 && mint === 0 && miao === 0) {
        window.location.reload();
      }
      return setTimeout(checkTimes, 1000);
    } else {
      $(".day span", this).text("0");
      $(".hour span", this).text("0");
      $(".mint span", this).text("0");
      return $(".miao span", this).text("0");
    }
  });
};

ressubmit = function(id) {
  return $.ajax({
    type: "post",
    dataType: "json",
    url: '/limit/lot/' + id,
...
createForm = function() {
  return gico.fBindSelect($('select'));
};

limitposts = function(id) {
  return $.ajax({
    type: "post",
    dataType: "json",
    ...
    success: function(msg) {
      if (msg.recode === 200) {
        return window.location.href = "/home/limit/" + id;
      } else {
        return alert(msg.reason);
      }
    }
...
loginsubmit = function() {
  ...
  return $.ajax({
    ...
    success: function(msg) {
      if (msg.recode === 200) {
        return window.location.href = "/user/regs";
      } else {
        return alert(msg.reason);
      }
    }
